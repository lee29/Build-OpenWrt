...
sed -i "/uci commit network/i\uci set network.lan.ipaddr='192.168.0.1'" $ZZZ                              # IPv4 地址(openwrt后台地址)
...
sed -i "/uci commit network/i\uci set network.lan.broadcast='192.168.0.255'" $ZZZ                         # IPv4 广播
...
echo "Lee29 Compiled in $(TZ=UTC-8 date "+%Y.%m.%d")" > package/base-files/files/etc/openwrt_gxqm     # 增加个性名字281677160

sed -i "/uci commit system/i\uci set system.@system[0].hostname='B70'" $ZZZ                       # 修改主机名称为OpenWrt-123
